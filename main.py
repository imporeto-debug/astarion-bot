...
from config import TMDB_API_KEY  # импортируем из config.py
...
async def tmdb_search(query: str, media_type="movie"):
    ...
    if not TMDB_API_KEY:
        print("⚠️ TMDb API ключ отсутствует (переменная TMDB_API_KEY пуста)")
        return None
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        try:
            async with session.get(url, params=params) as resp:
                if resp.status != 200:
                    print(f"TMDb HTTP ошибка: {resp.status}")
                    return None
                data = await resp.json()
                results = data.get("results", [])[:5]
                if not results:
                    print(f"TMDb нет результатов по запросу: {query}")
                    return None
                lines = []
                for item in results:
                    title = item.get("title" if media_type=="movie" else "name", "???")
                    ...
                    if overview and len(overview) > 100:
                        overview = overview[:100] + "…"
                    lines.append(f"• {title} ({year}) — рейтинг {rating}/10. {overview}")
                print(f"TMDb нашёл {len(lines)} результатов для запроса '{query}'")
                return lines
        except Exception as e:
        ...
        if found_topic:
            ...
            if found_topic in ("фильмы", "сериалы"):
                if not TMDB_API_KEY:
                    ...
                    add_to_history(message.channel.id, "assistant", reply_text)
                    await message.reply(reply_text, mention_author=False)
                    print("❌ TMDb ключ отсутствует (переменная пуста)")
                    return
                ...
                print(f"🔍 Поиск в TMDb: '{search_query}'")
                tmdb_results = await tmdb_search(search_query, media_type)

                if not tmdb_results:
                    # Упрощаем запрос: удаляем предлоги, союзы
                    words = search_query.split()
                    stop_words = ["с", "со", "в", "во", "на", "про", "и", "а", "но", "из", "по", "за", "от", "до", "без", "для", "о", "об", "при", "у", "к", "ко"]
                ...
                prompt = [
                    ...
                    {"role": "user", "content": (
                        ...
                        f"Автор — {'жена' if current_is_wife else 'не жена'}.\n"
                        f"Обращение к автору как '{address}'.\n"
                        "Сделай 3–5 рекомендаций в своём стиле, используя только данные из списка. "
                        "Каждый пункт — одно предложение с названием и кратким комментарием. "
                        "НЕ ВЫДУМЫВАЙ названия, которых нет в списке. Если список пуст — скажи, что ничего не нашёл."
                    )}
                ]
...
async def on_ready():
    ...
    if TMDB_API_KEY:
        print("✅ TMDb API ключ загружен из config.py")
    else:
        print("❌ TMDb API ключ НЕ загружен (переменная TMDB_API_KEY пуста)")
