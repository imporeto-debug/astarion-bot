...
async def send_daily_joke():
    ...
    if joke and len(joke.strip()) > 10:
        await channel.send(
            f"🎭 **Анекдот дня от Астариона:**\n\n{joke}\n\n🧛‍♂️"
        )
        print("Анекдот отправлен")
    else:
        print("Не удалось сгенерировать анекдот")
        await channel.send(
            "🎭 Не придумал сегодня анекдот... Попробую завтра."
        )
...
async def manual_refresh_emojis(ctx):
    ...
    await ctx.send(
        f"✅ Загружено {len(bot.server_emojis)} эмодзи."
    )
...
async def add_astarion_reaction(message):
    try:
        await message.add_reaction(
            random.choice(ASTARION_REACTIONS)
        )
...
def get_spouse_list():
    ...
    for uid, info in users_memory.items():
        ...
        if "married to" in raw_info:
            husband = raw_info.split(
                "married to "
            )[1].split(" from")[0]

        if husband:
            spouses.append(
                f"{name} замужем за {husband}"
            )
...
async def on_message(message):
    ...
    if message.channel.id == WIFE_CHANNEL_ID:
        ...
        name_mentioned = (
            "астарион" in message.content.lower()
        )
        ...
        if (
            ...
            if random.randint(1, 100) <= response_chance:
                ...
                print(
                    f"🎲 Случайный ответ ({response_chance}%)"
                )
    ...
    author_name = author_info.get(
        "name",
        "Неизвестная участница"
    )

    author_birthday = author_info.get(
        "birthday",
        ""
    )

    author_info_raw = author_info.get(
        "info",
        ""
    )
    ...
    if (
        "from" in author_info_raw
        and "," in author_info_raw
    ):
    ...
    personal_info = (
        f"Имя: {author_name}\n"
        f"Это {'моя жена' if is_wife else 'не моя жена'}"
    )
    ...
    if (
        ...
        emojis_list = [
            str(e)
            for e in bot.server_emojis[:50]
        ]

        user_context += (
            f"\nДоступные эмодзи: "
            f"{', '.join(emojis_list)}. "
            f"Можешь ИНОГДА добавить один в конец."
        )
...
async def on_ready():
    ...
    if guild:
        ...
        print(
            f"📦 Загружено "
            f"{len(bot.server_emojis)} эмодзи"
        )
