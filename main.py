...
- Long (5-6 sentences): when you need to explain a thought or tell a story
- NEVER mention hobbies unless directly relevant to the conversation
- NEVER be boring or overly verbose

You can discuss ANY real-world topics freely: politics, society, culture, news, everyday life. Have opinions. Be witty and sarcastic.

For jokes: any real-world subject, 2-4 sentences, no fantasy lore.
...
Длина ответа:
- ОЧЕНЬ КОРОТКИЕ (1-2 предложения): вопросы, восклицания, саркастичные реплики
- СРЕДНИЕ (3-4 предложения): обычные ответы, развитие темы
- ДЛИННЫЕ (5-6 предложений): когда нужно объяснить мысль, пошутить

Никогда не пиши "душно" — не растягивай мысль искусственно.

О хобби собеседника:
- НЕ упоминай хобби в каждом ответе
...
<JOKE_RULES>
Topics: ANY real-world subject. No fantasy lore required. Length: 2-4 sentences.
</JOKE_RULES>
...
<NO_INVENTION>
Astarion never invents movie titles, book titles, song names, or any specific real-world facts. If he lacks real information, he admits it, deflects with humor, or changes the subject gracefully. He does not generate fictional recommendations. When given a list of real items, he uses ONLY those items and does not add anything outside the list.
</NO_INVENTION>
<STYLE>
Speak only Russian.
...
async def on_message(message):
    ...
    if "посоветуй" in content_lower:
        ...
        if found_topic:
            ...
            search_query = message.content.replace("посоветуй", "").replace(found_topic, "").strip()
            if not search_query:
                # Если ничего не осталось, используем стандартный запрос из TOPIC_MAP
                search_query = TOPIC_MAP[found_topic]

            # Выполняем поиск через DuckDuckGo
            print(f"🔍 Поиск в DuckDuckGo: '{search_query}'")
            data = await duck_search(search_query)
            results = parse_results(data)

            if not results:
                # Нет результатов – отправляем DeepSeek для генерации отказа в стиле Астариона
                prompt_no_results = [
                    {"role": "system", "content": SYSTEM_PROMPT},
                    {"role": "user", "content": (
                        f"Сегодня: {datetime.now().strftime('%d-%m-%Y')}\n"
                        f"Пользователь попросил посоветовать {found_topic} по запросу: «{search_query}».\n"
                        f"К сожалению, по этому запросу ничего не найдено.\n"
                        ...
                        "- Предложить попробовать другой запрос (например, по жанру, году или более общую тему).\n"
                        "- Пошутить по поводу вкусов пользователя.\n"
                        "ВАЖНО: НЕ выдумывай названия фильмов/книг и т.п., которых нет в реальности. "
                        "Просто констатируй факт и реагируй в характере."
                    )}
                ]
            ...
            # Результаты есть – передаём их DeepSeek для оформления
            ...
            prompt = [
                ...
                {"role": "user", "content": (
                    ...
                    f"Автор — {'жена' if current_is_wife else 'не жена'}.\n"
                    f"Обращение к автору как '{address}'.\n"
                    f"Сделай 3–7 рекомендаций в своём стиле, используя только данные из списка. "
                    f"Каждый пункт — одно предложение с названием и кратким комментарием. "
                    f"НЕ ВЫДУМЫВАЙ названия, которых нет в списке."
                )}
            ]
...
async def on_ready():
    ...
    print("✅ Поиск выполняется через DuckDuckGo для всех категорий")
    if not daily_wife_message.is_running():
        daily_wife_message.start()
    if not holiday_task.is_running():
        holiday_task.start()
